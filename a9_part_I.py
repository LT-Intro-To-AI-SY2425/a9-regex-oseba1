...
pat = re.compile(r"^(\d+)\s([A-Za-z0-9\s]+)\s*\n([A-Za-z\s]+),\s([A-Za-z]{2})\s(\d{5}(-\d{4})?)$", re.IGNORECASE)
...
if __name__ == "__main__":
    ...
    print(f'number is: {address_matches.group(1)}!') # should print "number is: 2501"
    print(f'street is: {address_matches.group(2)}!') # should print "street is: Addison Street"
    print(f'city is: {address_matches.group(3)}!')     # should print "city is: Chicago"
    print(f'state is: {address_matches.group(4)}!')   # should print "state is: IL"
    print(f'zip is: {address_matches.group(5)}!')       # should print "zip is: 60618"
    assert address_matches.group(1) == '2501', "Incorrect address number"
    assert address_matches.group(2) == 'Addison Street', "Incorrect street"
    assert address_matches.group(3) == 'Chicago', "Incorrect city"
    assert address_matches.group(4) == 'IL', "Incorrect state"
    assert address_matches.group(5) == '60618', "Incorrect zip"
